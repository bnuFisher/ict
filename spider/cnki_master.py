...
url_list=[] #论文url队列 #建议写一个loop，分批次抓取
...
def getpagecontent(url_list):
    ...
    for url in url_list:
        ...
        try:
            ...
            miss_list.append(url) #建议不仅统计缺失情况，也使用重置方式再爬取一次第一次无效的url，可以将miss_list里的url写入文件保存
            continue
...
def writing(rows,count):
    ...
    with open('master_info.csv', 'a', newline='', encoding='utf-8') as f: #建议分批次写入
        f_csv = csv.DictWriter(f, headers)
        f_csv.writeheader() #后续写入时可以将这行注释掉
